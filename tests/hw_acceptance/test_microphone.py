...
class state_machine(KEYPAD):
    ...
    def key_press_cb(self, channel):
        ...
        if inputs == 0:
            logger.debug("No keypad change, mic_bits=0x%02x", self.last_inputs & 0x80)
            if ((self.last_inputs & 0x80) == 0) and self.mic_switch_status is True:
                logger.info("Mic Switch is now OFF")
                self.mic_switch_status = False
            if ((self.last_inputs & 0x80) == 128) and self.mic_switch_status is False:
...
def main():
    S = state_machine()
    data, _ = read_json()
    if data["speakers"]["test_report"]["left"] is False and data["speakers"]["test_report"]["right"] is False:
        ...
        if not S.mic_switch_status:
            logger.info("Mic switch is off, waiting for operator")
        ...
        summary["microphones"]["test_report"] = {}
    logger.debug("Microphone summary: %s", summary)
    update_json(summary)
    logger.info("Microphone test result: %s", S.test_result)
    if S.test_result:
        sys.exit(0)
    else:
        sys.exit(1)
