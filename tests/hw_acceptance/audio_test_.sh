...
aplay -D plughw:CARD=wm8960soundcard,DEV=0 -c 1 -f S16_LE mic_test_left.wav
# playback right channel of recorded audio 
aplay -D plughw:CARD=wm8960soundcard,DEV=0 -c 1 -f S16_LE mic_test_right.wav
# Please note that for some weird reason aplay -D hw:3,0 does not work for the
# last two files and only works for first file. However, aplay -D plughw:CARD=wm8960soundcard,DEV=0
# works!!!